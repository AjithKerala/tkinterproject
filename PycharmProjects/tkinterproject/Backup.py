...
def select_data(table):
    curtitem=table.focus()
    Value=table.item(curtitem)
    ...
    frame4 = tkinter.LabelFrame(frame3, text="Libary Membership info", width=700, height=400, relief="ridge",
                                borderwidth=10)
    ...
    listbook = ["RanddamUzham", "Nallukettu", "Mayyazhippuzhayude Theeragalill", "Brave New World", "The Great Gatsby",
                "The Catcher in thr Rye", "Great Expectation", "The Adventures of Hukleberry Finn"]
    ...
    refentry.insert(0, selectitem[0])
    title.insert(1, selectitem[1])
    fnentry.insert(2, selectitem[2])
    lnentry.insert(3, selectitem[3])
    adentry.insert(4, selectitem[4])
    pcentry.insert(5, selectitem[5])
    booktitleentry.insert(6, selectitem[6])

    dateofissentry.insert(7,selectitem[7])
    duedatentry.insert(8,selectitem[8])
    phno.insert(9, selectitem[9])
    priceentry.insert(10, selectitem[10])
    returndate.insert(11, selectitem[11])
    def update():
        nonlocal  mementry,refentry,title,fnentry,lnentry,adentry,pcentry,booktitleentry,dateofissentry,duedatentry,phno,priceentry,returndate,curtitem
        member = mementry.get()
        ref = refentry.get()
        tit = title.get()
        fnam = fnentry.get()
        ...
        pri = priceentry.get()
        tabel.item(curtitem,values=(selectitem[0],member,ref,tit,fname,lnam,addres,pcode,bkid,bktitle,daissu,due,ph,pri,redate))
        dbCursor=database.cursor()
        up="Update tkmainproject set Membertype='%s' Ref='%s'Title='%s' FirstName='%s' LastName='%s' Address='%s' PostCode='%s' BookTitle='%s',DateIssued='%s',Duedate='%s', Mobileno='%s', price='%s', Return='%s' where Ref='%s'"%(member,ref,tit,fnam,lnam,addres,pcode,bktitle,daissu,ph,pri,redate,selectitem[0])
        dbCursor.execute(up)
        mb.showwarning("Successfull",'Successfully worked')
        database.commit()
        database.close()
    ...
    upds = Button(root, text="Updates", width=25, bg="cyan", command=update)
    upds.place(x=1250,y=90)
    close = Button(root, text="Close", width=25, bg="cyan", command=root.destroy)
    close.place(x=1250,y=120)
...
upd=Button(btnframe,text="Update",width=25,bg="cyan",command=lambda:select_data(tabel))
